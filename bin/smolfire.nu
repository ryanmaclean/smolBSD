...
#   bhyve             nu bin/bhyve-smolfire-vm.nu              — launch smolfire bhyve guest
#   qemu              nu bin/qemu-smolfire-vm.nu               — launch smolfire qemu guest (HVF on Apple Silicon)
...
#
# Examples:
#   nu bin/smolfire.nu build --input FreeBSD-15.0-RELEASE-amd64-SMOLFIRE-VM.qcow2
#   nu bin/smolfire.nu test --image smolfire.qcow2                           # qemu, default
#   nu bin/smolfire.nu test --image smolfire.qcow2 --tpm                     # qemu + swtpm
#   nu bin/smolfire.nu test --image smolfire.qcow2 --arch arm64              # qemu arm64
#   nu bin/smolfire.nu test --image smolfire.raw --backend bhyve             # bhyve amd64
#   nu bin/smolfire.nu test --image smolfire.raw --backend bhyve --tpm       # bhyve + swtpm
...
#   nu bin/smolfire.nu ci-gate --run --image smolfire-amd64.raw --tpm
#   nu bin/smolfire.nu board-probe
#   nu bin/smolfire.nu swtpm --action start
#   nu bin/smolfire.nu bhyve --image smolfire-amd64.raw --tpm
#   nu bin/smolfire.nu qemu --image smolfire.qcow2 --arch arm64
...
def subcommand-table [] {
    [
        ...
        {subcommand: "test",               script: "bin/run-vm-tests.nu",              description: "Run the full smolfire VM acceptance test suite (--backend qemu|bhyve)"},
        ...
        {subcommand: "bhyve",              script: "bin/bhyve-smolfire-vm.nu",             description: "Launch smolfire bhyve guest with optional swtpm TPM"},
        {subcommand: "qemu",               script: "bin/qemu-smolfire-vm.nu",              description: "Launch smolfire qemu guest (HVF on Apple Silicon / KVM on Linux)"},
        ...
        {subcommand: "first-boot",         script: "bin/first-boot.nu",                description: "Run first-boot provisioning inside a booted smolfire guest"},
...
def print-help [] {
    print "smolfire — smolfire toolchain CLI"
    print ""
...
def --wrapped main [
    ...
    match $sub {
        ...
        "bhyve" => {
            ^nu --no-config-file bin/bhyve-smolfire-vm.nu ...$args
        }

        "qemu" => {
            ^nu --no-config-file bin/qemu-smolfire-vm.nu ...$args
        }
