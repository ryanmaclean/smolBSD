...
plan: "07"
subsystem: infra
tags: [github-actions, freebsd, qcow2, tpm2-tools, smolbsd, ci, build-image, pop4090]

requires:
  - phase: 03-tpm-2-0-measured-boot
    provides: "03-03: smolbsd-amd64-tpm.qcow2 at /home/studio/smolbsd-ci/ on pop4090 — confirms the build approach and exact make invocation that works"
  - phase: 03-tpm-2-0-measured-boot
    provides: "03-04 / 03-05: tpm-vm-test.yml and CI runner registration confirming pop4090-amd64 runner labels"

provides:
  - ".github/workflows/build-image.yml — CI workflow that rebuilds the smolBSD TPM qcow2 on pop4090 via workflow_dispatch or on push to SMOLBSD/smolbsd-qemu.conf"
...
patterns-established:
  - "Workflow copies configs from checkout into pre-existing freebsd-src tree on pop4090 runner — no full src clone needed on each run"
...
**GitHub Actions workflow build-image.yml targeting pop4090-amd64 runner, building smolBSD qcow2 via make cloudware-release KERNCONF=SMOLBSD with tpm2-tools baked in, saving artifact + SHA256 manifest to /home/studio/smolbsd-ci/**
...
- Workflow copies `sys/amd64/conf/SMOLBSD` and `release/tools/smolbsd-qemu.conf` from checkout into `$FREEBSD_SRC` on pop4090
...
- `.github/workflows/build-image.yml` — 182-line GitHub Actions workflow: workflow_dispatch + push triggers, pop4090-amd64 runner, config copy, cloudware-release build, artifact promotion, SHA256 manifest
...
## Known Stubs
None — the workflow is complete and references no placeholder paths or hardcoded mock values. All paths are the actual pop4090 paths confirmed in plan-03.

## User Setup Required
None — pop4090 runner is already registered (confirmed in plan-04/05 context). No new external configuration required.
