...
| **SMOLFIRE** (microVM) | **543 ms to shell under Firecracker** (476 ms QEMU microvm) | **37 MiB — one PVH ELF is the whole OS** (kernel + static /rescue MFS root) | Rump-kernel spirit: no bootloader, no disk, no pkgbase; `sys/amd64/conf/SMOLFIRE` + `bin/build-smolfire.sh` |
